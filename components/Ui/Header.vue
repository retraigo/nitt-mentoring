<template>
    <header class="flex absolute lg:relative top-0 left-0 bg-nitMaroon-650 z-50 h-screen">
        <div class="flex flex-col justify-between w-full md:max-w-7xl lg:max-w-[100rem]">
            ...
            <div :class="`z-40 p-4 top-0 bg-nitMaroon-650 flex-grow overflow-y-auto flex flex-col ${route.path !== `/login` ? `justify-between` : `justify-end`} items-start gap-8 uppercase text-sm text-white transition-all duration-500 ease-in-out transform h-screen lg:h-auto overflow-y-hidden lg:overflow-y-visible overflow-x-hidden w-full ${navState ? `max-w-lg` : `max-w-[4rem]`
                }`">
                <div v-if="userStore.loggedIn" class="flex flex-col gap-8">
                    <div class="mt-8" />
                    <NuxtLink v-for="route in MainMenu.filter(x => x.level <= userStore.level && (userStore.level === 3 ? x.level === 3 : true))" :to="route.action"
                        :key="route.key"
                ...
                <!-- If not logged in -->
                <NuxtLink v-else to="/login" title="Login"
                    :class="`flex flex-row items-center gap-2 transition duration-200 ease-in-out border-b-2 border-transparent ${navState ? `hover:border-nitMaroon-300` : `justify-center w-full`}`">
                    <svg class="block w-5 h-5 stroke-2" xmlns="http://www.w3.org/2000/svg" fill="none" stroke="#fccef1"
                        viewBox="0 0 24 24" aria-hidden="true">
                        <path class="transition-all duration-500 transform ease-in-out" stroke-linecap="round"
